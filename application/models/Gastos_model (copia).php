...
class Gastos_model extends CI_Model {
	...
	{
		$query = $this->db->query('SELECT outgo.id,  date,  amount,  detail,  type_outgo_id, type_outgo.name AS type_outgo,  user_id,  regional_id, regional.name AS regional_name FROM outgo INNER JOIN type_outgo ON type_outgo.id = outgo.type_outgo_id INNER JOIN regional ON regional.id = outgo.regional_id'.$this->regional_filter);
		return $query;
	}
	...
	{
		$query = $this->db->query('SELECT id, title, description, file_name FROM outgo_img WHERE regional_id = ?',array($_SESSION['regional']));
		return $query;
	}
