...
class Login_model extends CI_Model {
	...
	public function signin($array) {
		...
		$sql = 'SELECT user.id, user.name, lastname, email, pass, user_profile_id, regional_id, block, user.deleted, regional.name AS regional_name FROM user INNER JOIN regional ON regional.id = user.regional_id WHERE deleted != 1 AND email = ? LIMIT 1';
	...
	{
		$query = $this->db->query('SELECT fail FROM user WHERE email = ? LIMIT 1', $email);
		return $query;
	}
	...
	{
		return $this->db->query('SELECT block FROM user WHERE email = ?',$email);
	}
