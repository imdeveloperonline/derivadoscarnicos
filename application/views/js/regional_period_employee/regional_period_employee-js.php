		...
		<script>
			function fillTable(id) {
				...
				$.ajax({
					url: "<?= base_url() ?>regionales/get_last_employee_period/"+id,
		...
		<script>
			...
			function new_period() {
				...
				var params = {
		                ...
		                "regional_employees_id" : $('input[name="employee_id"]').val()
		        };

				$.ajax({
                      data:  {data:params},
                      url:   '<?= base_url(); ?>regionales/set_new_employee_period',
                      type:  'post',
                      beforeSend: function () {
		...
		<script>
			...
			function set_modal (id) {
				$('input[name="end_period_id"]').val(id);
		...
		<script>
			...
			function end_period() {        
		        ...
		        var params = {
		          "id" : $('input[name="end_period_id"]').val(),
		          "date_end" : $('input[name="date_end"]').val(),
		          "employee_id" : $('input[name="employee_id"]').val()
		        }

		        var n = $('input[name="end_period_id"]').val();

		        $.ajax({
		              data:  {data:params},
		              url:   '<?= base_url(); ?>regionales/end_employee_period',
		              type:  'post',
		              success:  function (response) {
		                      $("#message").remove();
		                      $(response).appendTo('#resultado');
		...
		<script>
			...
			function change_end_date (id,date_end) {
				
				$("#date_end_"+id).html(date_end);
				$("#calendar_button_"+id).html("Finalizado");
			}
		</script>