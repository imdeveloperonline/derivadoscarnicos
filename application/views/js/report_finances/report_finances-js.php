		...
		/*$(function(){
			$("select[name='department']").change(function(){
				...
				$.ajax({
	                  ...
	                  url:   '<?= base_url(); ?>reportes/get_regionals_by_department',
	                  type:  'post',
	                  success:  function (response) {
	                  	$("select[name='regional").html(response);

	                  }
				});

				if($("#regionals").css('display') == "none") {
					$("#regionals").toggle(250);
				}
			});
		});*/
	...
	<script>
		...
		function send_form() {
			...
			$.ajax({
                  data:  {data:params},
                  url:   '<?= base_url(); ?>reportes/get_finances_report',
                  type:  'post',
                  beforeSend: function () {
                  	$("#report").html(" ");
