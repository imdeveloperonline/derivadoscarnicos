		...
		<script>
			function fillTable() {
				...
				$.ajax({
					url: "<?= base_url() ?>bodega/get_last_dispatch_regional",
					success: function (response) {
		...
		<script>
	      ...
	      function new_dispatch_regional() {

	         var params = $("#dispatches-regional-form").serialize();
	          
	         $.ajax({
	                data:  {data:params},
	                url:   '<?= base_url(); ?>bodega/set_dispatch_regional',
	                type:  'post',
	                beforeSend: function () {
