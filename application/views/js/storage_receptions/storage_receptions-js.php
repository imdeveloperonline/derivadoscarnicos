				...
				if(method == 3) {
					...
					if(supplier != false) {
						...
						if(method == 2) {
							...
							$.ajax({
								url: '<?= base_url() ?>finanzas/get_active_adv_supplier',
								type: 'post',
								data: {supplier_id: supplier_id},
								success : function(response) {
									$("#message-form").remove();
									if(response == 1) {
										 $('<div id="message-form" class="alert alert-block alert-warning"><a class="close" data-dismiss="alert" href="#">×</a><h4 class="alert-heading"><i class="fa fa-exclamation-triangle"></i> Alerta</h4><p>Este proveedor tiene un anticipo vigente. No podrá registrar un crédito mientras tenga un anticipo vigente.</p></div>').appendTo('#form-alert').hide().fadeIn('slow');
									} 
								},
								error : function (error) {
									alert(JSON.stringify(error));
